...
export function ClientContractView({ contract, profile, auditLogs, startProposingRevision }: ClientContractViewProps) {
  return (
    <div className="lg:col-span-8 bg-white shadow-md border border-slate-200/50 rounded-3xl p-6 md:p-8 text-left print:shadow-none print:border-none print:p-0">
      ...
      <div className="flex flex-col gap-6 pb-6 border-b border-slate-100">
        <div className="flex justify-between items-start gap-4">
          <div className="flex items-center gap-4">
            {profile?.logoUrl && (
              <img src={profile.logoUrl} alt="Logo" className="h-12 w-12 object-contain rounded-xl border border-slate-100 bg-white" />
            )}
            <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
          <div className="rounded-xl border border-slate-100 p-4">
            ...
            {contract.freelancerRfc && (
              <div className="text-slate-500 mt-1.5 leading-relaxed font-light">
                <p>RFC: <span className="font-semibold text-slate-700">{contract.freelancerRfc}</span></p>
          ...
          <div className="rounded-xl border border-slate-100 p-4">
            ...
            <span className="font-bold text-slate-800 mt-1 block">{contract.clientName}</span>
            <span className="text-slate-500 mt-0.5 block">{contract.clientEmail}</span>
            {contract.clientRfc ? (
              <div className="text-slate-500 mt-1.5 leading-relaxed font-light">
                <p>RFC: <span className="font-semibold text-slate-700">{contract.clientRfc}</span></p>
      ...
      <div className="py-6 border-b border-slate-100 flex flex-col gap-3">
        ...
        <p className="text-sm leading-relaxed text-slate-600 font-light whitespace-pre-wrap">{contract.scopeDescription}</p>
      </div>

      {/* Clause list */}
      <div className="py-6 border-b border-slate-100 flex flex-col gap-4">
        <div className="flex items-center justify-between">
      ...
      <div className="py-6 flex flex-col gap-4">
        ...
        {contract.acceptedAt ? (
          <div className="rounded-xl border border-purple-500/20 bg-purple-500/5 p-4 text-xs flex flex-col gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-slate-500 mt-1 font-light">
              <p>Firmante: <span className="font-semibold text-slate-700">{contract.acceptedByName}</span></p>
              <p>Dirección IP: <span className="font-semibold text-slate-700">{contract.acceptedIp}</span></p>
              <p>Fecha/Hora: <span className="font-semibold text-slate-700">{contract.acceptedAt ? new Date(contract.acceptedAt).toLocaleString('es-MX') : ''}</span></p>
            </div>
          </div>
        ) : (
          <div className="rounded-xl border border-slate-200 border-dashed p-6 text-center text-slate-400 text-xs font-light">
        ...
        {(contract.status === 'accepted' || contract.status === 'completed') && contract.freelancerAcceptedAt ? (
          <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 text-xs flex flex-col gap-3">
            <div className="flex justify-between items-start gap-4">
              <div className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-slate-500 mt-1 font-light">
                  <p>Validador: <span className="font-semibold text-slate-700">{contract.freelancerAcceptedByName}</span></p>
                  <p>Dirección IP: <span className="font-semibold text-slate-700">{contract.freelancerAcceptedIp}</span></p>
                  <p>Fecha/Hora: <span className="font-semibold text-slate-700">{contract.freelancerAcceptedAt ? new Date(contract.freelancerAcceptedAt).toLocaleString('es-MX') : ''}</span></p>
                </div>
              </div>
              {profile?.signatureUrl && (
                <img src={profile.signatureUrl} alt="Firma Freelancer" className="max-h-12 object-contain bg-white rounded-lg p-1 border border-slate-100" />
              )}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 text-xs flex flex-col gap-2">
            ...
            <p className="text-slate-500 leading-relaxed font-light mt-1">
        ...
        {contract.contractHash && (
          <div className="rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-5 text-xs flex flex-col gap-3 mt-2">
            ...
            <div className="bg-slate-100 p-2.5 rounded-lg border border-slate-200/40">
              ...
              <span className="font-mono text-xs text-slate-600 break-all select-all font-light">{contract.contractHash}</span>
            </div>
          </div>
        )}
      </div>
      ...
      {auditLogs.length > 0 && (
        <div className="py-6 border-t border-slate-100 mt-6 flex flex-col gap-4">
                ...
                <li key={log.id}>
                  <div className="relative pb-8">
                    ...
                    <div className="relative flex space-x-3 items-start">
                      <div>
                        <span className={`h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-white ${ log.action === 'created' ? 'bg-slate-100 text-slate-500' : log.action === 'client_signed' ? 'bg-purple-100 text-purple-600' : log.action === 'freelancer_accepted' ? 'bg-emerald-100 text-emerald-600' : log.action === 'milestone_requested' ? 'bg-amber-100 text-amber-600' : log.action === 'milestone_transferred' ? 'bg-blue-100 text-blue-600' : 'bg-indigo-100 text-indigo-600' }`}>
