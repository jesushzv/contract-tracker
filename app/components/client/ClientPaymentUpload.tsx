...
export function ClientPaymentUpload({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isFreelancer) {
      handleMarkAsTransferred(e);
      return;
    }

    const hasReceipt = receiptFileType === 'file' ? !!receiptFileName : !!receiptUrl.trim();
    if (!hasReceipt) {
  ...
  const handleProceedWithoutEvidence = () => {
    ...
    handleMarkAsTransferred(dummyEvent);
  };
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md print:hidden">
        <div className="relative glass rounded-3xl p-6 max-w-md w-full animate-in zoom-in-95 duration-200 text-left bg-white shadow-2xl border border-indigo-500/20">
          ...
          <h3 className="text-xl font-bold flex items-center gap-2 text-indigo-500">
            ...
            {isFreelancer ? "Registrar Pago (Notificar Transferencia SPEI)" : "Notificar Transferencia SPEI"}
          </h3>
          <p className="text-xs text-slate-500 mt-2 leading-relaxed">
            {isFreelancer 
              ? "Registra los detalles del pago de este hito para llevar el control. Puedes ingresar una referencia o comprobante de pago opcionalmente."
          ...
          <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2">
                Clave de Rastreo SPEI / Referencia (Opcional)
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2">
                Método de Comprobante (Opcional)
              </label>
      ...
      {showSuggestionModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md print:hidden animate-in fade-in duration-200">
          <div className="relative glass rounded-3xl p-6 max-w-md w-full animate-in zoom-in-95 duration-200 text-left bg-white shadow-2xl border border-indigo-500/20">
            <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
              <AlertCircle className="h-5 w-5 text-amber-500" />
              ¿Deseas adjuntar un comprobante?
            </h3>
            <p className="text-xs text-slate-500 mt-2 leading-relaxed">
              {!trackingReference.trim() 
                ? "No has ingresado una clave de rastreo ni seleccionado un comprobante de pago. Aunque no es obligatorio, proporcionar esta información ayuda a tu freelancer a verificar el pago más rápido."
                : "No has adjuntado una captura, PDF o enlace de tu comprobante de transferencia. Aunque no es obligatorio, adjuntar esta evidencia ayuda al freelancer a confirmar tu pago con mayor rapidez."
              }
            </p>
            <div className="flex gap-3 justify-end mt-6">
              <button
                ...
                className="rounded-xl px-4 py-2 text-xs font-semibold text-indigo-650 bg-indigo-50 hover:bg-indigo-100 transition-colors"
              >
                Agregar Comprobante
              </button>
              <button
                type="button"
                onClick={handleProceedWithoutEvidence}
                className="rounded-xl bg-slate-200 hover:bg-slate-300 text-slate-700 px-4 py-2 text-xs font-semibold transition-colors"
              >
