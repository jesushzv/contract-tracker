    ...
    <div className="mx-auto w-full max-w-4xl px-4 py-12 sm:px-6 lg:px-8 flex-grow flex flex-col gap-6 text-slate-800 dark:text-slate-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 flex flex-col gap-4">
          <form onSubmit={handleVerify} className="glass rounded-3xl p-6 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950/40 flex flex-col gap-4">
            <div className="flex flex-col gap-1.5 text-left">
              ...
              <textarea
                ...
                className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white/40 dark:bg-slate-900/40 px-4 py-2.5 text-sm font-mono focus:border-indigo-500 focus:outline-none dark:text-white transition-all resize-y"
              />
            </div>
            ...
            <div className="flex flex-col gap-1.5 text-left">
              ...
              <input
                ...
                className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white/40 dark:bg-slate-900/40 px-4 py-2.5 text-sm font-mono focus:border-indigo-500 focus:outline-none dark:text-white transition-all"
              />
            </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="glass rounded-3xl p-6 border border-indigo-500/20 bg-indigo-500/5 text-left flex flex-col gap-4">
            <h3 className="font-bold text-sm text-slate-900 dark:text-white">¿Cómo funciona?</h3>
          ...
          {computedHash && (
            <div className="glass rounded-3xl p-6 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950/40 text-left flex flex-col gap-3 animate-in fade-in duration-300">
              <span className="text-2xs font-extrabold text-slate-400 uppercase tracking-widest">Sello Calculado</span>
              <div className="bg-slate-50 dark:bg-slate-900 p-3 rounded-xl border border-slate-100 dark:border-slate-800/80 font-mono text-2xs break-all leading-normal select-all">
                {computedHash}
              </div>
              <button
                onClick={copyComputedHash}
                className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-slate-200 dark:border-slate-800 px-3 py-1.5 text-3xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-900 transition-all cursor-pointer self-start"
              >
          ...
          {verified !== null && (
            <div className={`glass rounded-3xl p-6 border text-left flex flex-col gap-2 animate-in slide-in-from-bottom-2 duration-300 ${verified ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-red-500/20 bg-red-500/5'}`}>
              <h4 className={`font-black text-sm ${verified ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-650 dark:text-red-400'}`}>
                {verified ? "✓ Documento Auténtico" : "✗ Firma Inválida"}
              </h4>
              <p className="text-2xs leading-relaxed text-slate-550 dark:text-slate-350">
                {verified 
