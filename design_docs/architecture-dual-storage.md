...
*   **Behavior**: When a user is in demo mode, all CRUD transactions and validation checks are simulated entirely in-browser. A cookie (`demo_mode=true`) is also written (both instantly on button click, and by the Next.js middleware upon seeing `?demo=true`) to sync state across route redirects and Next.js middleware. This sandbox session is self-contained: the cookie and corresponding storage session are automatically cleared by the global navigation header when the user exits the app pages and returns to the public marketing site (e.g., homepage, FAQ, plans).
